...
# --- إعداد خادم ويب Flask لمنصة Render ---
app = Flask(__name__)

@app.route('/')
def home():
    return "البوت يعمل بنجاح على سيرفر Render السحابي! 🚀"
...
async def video_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text("❌ عذراً! الرابط محمي أو غير مدعوم حالياً في السيرفر.")
    finally:
...
async def mp3_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text("❌ فشل استخراج الـ MP3 بسبب حماية المنصة.")
    finally:
        try: await status_msg.delete()
        except: pass
...
def main():
    ...
    # تشغيل Flask في خلفية مستقلة (Thread) لإرضاء سيرفر Render
    flask_thread = threading.Thread(target=run_flask)
    flask_thread.daemon = True
    flask_thread.start()

    # تشغيل البوت بنظام Polling المستقر
    application = Application.builder().token(TOKEN).build()
    application.add_handler(CommandHandler("start", start))
    application.add_handler(CommandHandler("video", video_command))
    application.add_handler(CommandHandler("mp3", mp3_command))
    
    print("[+] البوت خادم الويب يعملان الآن على Render...")
